...
class Game(tk.Frame):
    ...
    def game_loop(self):
        ...
        if num_bricks == 0:
            self.ball.speed = 0
            self.show_message('KERENN BANGET LU BRO')
        elif self.ball.get_position()[5] >= self.height:
            self.ball.speed = 0  # Hentikan bola
            self.lives -= 1
            if self.lives < 0:
                self.show_message('ADUHHH SAYANG SEKALII')
            else:
                # Tampilkan "SILAHKAN COBA LAGI BROO" saat kehilangan nyawa
                emoji = self.draw_text(300, 200, 'SILAHKAN COBA LAGI BROO', size='25')
